...
//!
//! Drive an Onboarding from P1 with both attestors accepting up front, then
//! hard-kill P2 right after its attestor row reaches `inprogress`, restart
...
pub async fn run(f: &mut Fixture) -> anyhow::Result<()> {
    ...
    let prefix = chaos::fresh_prefix("resume-attestor");
    ...
    // Both attestors accept up front (so P2 is mid-flight when we kill it).
    let p2_inv =
    ...
    // Wait for the inprogress attestor row, then capture its synthesized
    // instance_name so we can refer to it across the restart.
    let p2_db = f.db_path(2);
    let p2_attestor_instance = wait_for_attestor_instance(&p2_db).await?;

    // Capture created_at so we can assert the row is reused, not recreated.
    let created_before = db::workflow_run_created_at(&p2_db, &p2_attestor_instance, "Attestor")
        .await?
        .ok_or_else(|| anyhow::anyhow!("missing created_at for P2 attestor row"))?;
    ...
    chaos::poll_until(Duration::from_secs(240), || async {
        Ok(matches!(
            db::workflow_run_status(&p2_db, &p2_attestor_instance, "Attestor")
                .await?
                .as_deref(),
            Some("completed")
        ))
    ...
    let row_count = db::count_workflow_run_rows(&p2_db, &p2_attestor_instance, "Attestor").await?;
    anyhow::ensure!(row_count == 1, "expected 1 attestor row, got {row_count}");
    let created_after = db::workflow_run_created_at(&p2_db, &p2_attestor_instance, "Attestor")
        .await?
        .ok_or_else(|| anyhow::anyhow!("missing created_at for P2 attestor row post-restart"))?;
    anyhow::ensure!(
    ...
    chaos::say("G2", "attestor resume verified (row reused, completed)");
    chaos::dismiss_p1(f, &instance).await;
    Ok(())
}

/// Spin until an `inprogress` Onboarding attestor row appears at `db_path`,
