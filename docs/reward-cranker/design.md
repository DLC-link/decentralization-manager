# CIP-104 Reward Cranker (Mode A) — Design
...
- **Consortium member** — one of the m-of-n orgs that jointly govern a decentralized party. Each wears **two hats**: a *decision-maker* co-governing the decparty (a business entity, L1), and a *node operator* whose participant + DecMan instance is a software agent (L2) — here, the node that runs the cranker. Same org, two layers.
...
*This design:*
- **Cranker** — the per-node background automation this doc specifies (a member node's L2 agent).
- **Proposer / Confirmer** — the cranker's two roles: propose a batched assignment; validate + auto- confirm proposals that match the on-ledger split.
...
Division of labour (per Robert, 2026-07-14): both Mode A and Mode B are selectable per decparty via a **shared DAML config template Robert is building**; Robert owns Mode B + that template; this increment is the **Mode A cranker**.
...
| **member node** running the cranker | each member org | its **DecMan instance** (the cranker) | its member party |
...
Every design choice below follows from this principle. **Writing rule for this doc:** a party id is never the grammatical subject of an action verb. An action is always done by an L2 *agent* (a node, a wallet, the cranker) on behalf of an entity; "as the decparty" names the *capacity* an action is authorized in, not an actor. If a sentence says "the decparty does X," that's a bug in the sentence.

## 3. Scope

**In scope**
- A new `GovernableAction` DAML template `AssignRewardBeneficiaries` wrapping `RewardCoupon_AssignBeneficiaries`.
- A new Rust background-automation module in `crates/decman` — the cranker — as **two roles**: a **proposer** (discovers coupons, proposes a batched assignment) and a **confirmer** (validates the proposal against on-ledger config and auto-confirms). Each member node runs an instance.
...
2. **Decparty configured for Mode A.** Modes are selectable per decparty via Robert's shared template. The cranker runs only where the configured mode is A (else Mode B's minting races it). Depends on the template exposing the mode — dependency (§14).
...
```
        per-node cranker = a member node's L2 agent (new background task)
        ┌──────────────────────────────────────────────┐
...
**Interface point (depends on Robert's shared template):** the cranker must read the *effective* split from a single authoritative on-ledger source. Preferred: the shared config template exposes the resolved `[RewardBeneficiary]` (or mode + inputs the cranker composes). Fallback: compose from `InstrumentConfiguration` + `AppRewardConfiguration` via the same `getBeneficiaries` logic. **This is the one blocking dependency to pin with Robert before implementation** — see §14.

## 9. Rust automation module (the cranker = a member node's L2 agent)

New module `crates/decman/src/server/reward_cranker/` (mirrors existing background-task style; registered via `tokio::spawn` in `start_server`, like the Canton sync loop).
...
- **Not** gated on an active `FeaturedAppRight`: the FAR governs whether *new* coupons accrue, not whether *existing* unassigned coupons can be assigned. The cranker acts on any live unassigned coupon whose `provider` is the decparty, even if the FAR has lapsed.
...
- **Non-custodial.** CC never lands with the decparty; assignment routes each coupon to a beneficiary party, whose own agent mints it. The cranker moves no funds.
- **No standing off-ledger secret.** The split lives on-ledger; the cranker reads it, never a node config file.
...
1. **Config interface (§8):** does the shared template expose the resolved `[RewardBeneficiary]`, or does the cranker compose it from `InstrumentConfiguration` + `AppRewardConfiguration` — and does the operator 20% cut apply on the V2 assign path? Determines the confirmer's validation source. *Blocking for implementation.*
...
3. Whether the shared template also carries the **mode (A/B)** selector the cranker gates on.
...
- **M3 — auto-confirmation engine + cranker:** the action-agnostic, default-deny auto-confirm engine (policy interface + background loop) with `AssignRewardBeneficiaries` as the sole enrolled policy; proposer + confirmer roles, effective-split read, TTL-watermark batching, bounded proposal expiry; Rust unit tests.
