...
- **Execution requires every confirmation to come from a current member.** `GovernanceRules_ExecuteConfirmedAction` asserts `confirmer ∈ members` across the whole list *at execute time* — it does not filter. Passing a removed member's confirmation aborts the execute. Leave it out of the list and re-submit; you lose that vote, so you may dip below `threshold` and need a fresh confirmation from a current member. The self-management path differs: `GovernanceRules_ExecuteGovernanceAction` *does* filter non-members out before counting, because `additionalProposers` may create self-confirmations as proposal markers.
...
- **An expired confirmation aborts execution; it is not skipped.** `GovernanceConfirmation_Consume` rejects any confirmation past its `expiresAt`, so the executor must leave stale ones out of the list it passes. `GovernanceRules_ExpireConfirmation` cleans them off the ledger.
