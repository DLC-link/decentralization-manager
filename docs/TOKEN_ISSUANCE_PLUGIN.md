...
**Alternatives if rejected.** (a) recipient co-signs `MintProposal` — consent-based, requires recipient online at propose time; (b) `MintPreapproval` contract — recipient opts in once, committee mints autonomously thereafter; (c) a different factory implementation that does not demand owner authority — speculative, depends on whether the registry offers one.
