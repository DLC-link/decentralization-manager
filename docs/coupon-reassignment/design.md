...
- **Coupon expires before assignment commits:** it leaves the ACS; the `Delegation_Assign` exercise for a chunk containing it fails cleanly (all-or-nothing); the chunk is retried smaller and the survivors go through, with anything still unassigned picked up by the next tick. Assigning on first sight leaves the whole 36h TTL as margin.
- **Coupon assigned by another node mid-tick:** same all-or-nothing failure; harmless. The chunk halves down to single coupons, and once a few of those fail the tick ends so the next one re-reads a fresh view instead of retrying a set another node already took.
...
- **Chunk size:** bounded by output creates, not a fixed coupon count (§9, step 4), and halved on a failed submission so an unmeasured transaction ceiling self-corrects. A tick drains its whole assignable set, so a long interval cannot by itself build a backlog.
...
- **Rust unit:** `ProposalType::SetupCouponReassignmentDelegation` serialization round-trip (copy the `build_proposal_*_shape` test shape); `validate()` rejects empty set / bad percentages / >cap (exact `DamlDecimal`, via `validate_reward_beneficiaries`); selection (`select_assignable` — minting-margin, most-urgent-first, no truncation) and chunk sizing (`chunk_size` — creates-based, never 0); the per-tick gating (zero delegations → no-op; two → refuse; node-not-assigner → skip).
...
- **M3 — per-round automation:** the background loop — read active delegation, scan unassigned coupons, `select_assignable` (minting-margin), drain the set in creates-sized chunks via `Delegation_Assign` as a plain command; per-tick gating; Rust unit tests.
