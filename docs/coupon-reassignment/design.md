...
- **`Delegation_Assign`** — the delegation's nonconsuming choice: an assigner supplies the target coupons and an `extraArgs` passed through to splice; the choice assigns the coupons to the **baked-in** split. The caller never supplies the beneficiaries — that is the security boundary (§12).
...
2. **Enablement — the decparty has an active `CouponReassignmentDelegation`.** There is no mode selector. The automation runs for a decparty **iff** that decparty has one (presence = on; absence = skip; >1 = use the newest and warn — see §11). Mode B (the one-shot collection path, PR #256) is a separate approach that must not run on the same decparty — the two would compete for the same unassigned coupons.
...
7. **The DSO mints the decparty's coupons with `providerIsObserver = true`.** Co-hosting (6a) is necessary but not sufficient. `RewardCouponV2` sets `observer (if providerIsObserver then [provider] ++ optionalToList beneficiary else [])`, so a coupon minted with the flag false is **not in the decparty's ACS at all** — this automation cannot see it, cannot assign it, and it expires. splice mints false when the provider's vetting state does not allow it, and DSO automation later tries to add the provider as an observer if that state changes, unless the coupon expired first. **Not verifiable from here and not fixable here:** the observer set is decided by the DSO at mint time, so this is a precondition on the environment rather than a property of this design. Its only local symptom is that `run_reassign_once` finds nothing, and that is **indistinguishable from a decparty that genuinely earned none** — an invisible coupon is not in the ACS, so both report `visible = 0` on the `no assignable coupons this tick` line. That count separates "coupons present but none assignable yet" from "nothing visible at all", and nothing more; deciding between "earned none" and "cannot see them" needs a signal from outside this node, such as the DSO's own record of what it minted for this provider.
...
```
   ...
   Every member node runs this each tick; running on several is safe (§10):
     ...
     3. select every assignable coupon (a small margin before expiry)
...
- **Decoded ACS reads:** `for_each_active_created` (decoded `GetActiveContracts`, following the `fetch_proposal_infos` decode pattern) — used to read the active delegation and the unassigned coupons. (Not `query_contracts_by_template`, which is blob-only, nor `get_contracts`, which is metadata-only.) The coupon interface is `#splice-api-reward-assignment-v1:Splice.Api.RewardAssignmentV1:RewardCoupon`. The read **streams**: it hands each decoded `Record` to a visitor and drops it before reading the next, so a caller holds only what it folds out. A verbose `Record` carries its field labels as well as its values, and the coupon ACS is unbounded, so collecting the records and narrowing afterwards made peak memory track the whole ACS instead of the result — that OOMKilled a devnet node at its 2Gi limit every few minutes (#413). A bounded per-decparty template read (the delegation) may still collect, via `bounded_created_records`.
...
**Config / gating**
- Enablement is the **presence of an active `CouponReassignmentDelegation`** for the decparty (§8) — there is no mode gate. None ⇒ no-op (log and skip); more than one ⇒ use the newest and warn (§11). The split is read from that contract, not from node config.
...
1. **Read the active delegation.** Load `CouponReassignmentDelegation` for the decparty via `bounded_created_records`. Zero ⇒ no-op; more than one ⇒ take the newest by created-event offset and warn. This node's member party must appear in `assigners` (else it cannot assign — log and skip).
2. **Query unassigned coupons:** active `RewardCouponV2` where `provider = decparty`, `dso` is the delegation's DSO, and `beneficiary = null`, ordered by `expiresAt`. (Minted/consumed coupons archive out of the ACS, so "active + beneficiary = null" suffices — no separate "already assigned" bookkeeping.) The read goes through the `RewardCoupon` **interface** but admits only contracts whose creating template is `Splice.Amulet:RewardCouponV2`. That restriction is load-bearing: `Delegation_Assign` fetches the primary as the concrete `RewardCouponV2`, so any other implementation of the interface would pass the reader and fail the exercise, and sorted most-urgent-first it would head the batch on every tick. The reader must admit exactly what the DAML accepts, or a second implementation shipping is enough to wedge the engine — no attacker required.
3. **Select the assignable coupons** (`select_assignable`): every unassigned coupon is eligible as soon as we see it, except those within `reward_min_expiry_margin_secs` of expiry. That margin guards **our own submission** — `Delegation_Assign` is all-or-nothing, so a coupon expiring between the ACS read and the commit fails its whole chunk — and is sized to submission latency (default 2 min), **not** to reserve minting time for the beneficiary: withholding a coupon guarantees nobody mints it, while assigning it late still lets the beneficiary try, since the per-beneficiary coupons inherit the original expiry. For the same reason there is deliberately **no minimum-age gate**: assignment neither consumes the coupon nor shortens its life. Selection does **not** truncate — see step 4.
4. **Assign the whole set, in chunks.** A tick drains everything it selected, submitting successive `Delegation_Assign` transactions, rather than assigning one batch and waiting for the next tick. This keeps **throughput independent of the tick interval**, so the interval is a latency/cost knob rather than a safety-critical one, and a backlog cannot grow just because the interval is long. The chunk bounds one *transaction*, not a tick's work: it is sized in **output creates** (`chunk_size = reward_max_creates / beneficiary_count`, config-tunable), because one assign creates `coupons × beneficiaries` contracts — a fixed coupon count would be `beneficiary_count`-times looser for a wide split than a narrow one. Coupons go most-urgent-first, so any partial failure sheds the coupons with the most time left. Cadence: a periodic tick (a few times/day), not per-round — coupons can be assigned any time before mint/expiry, which minimizes tx cost.
5. **Submit each chunk:** exercise `Delegation_Assign { assigner = this node's member party, primaryCoupon, additionalCoupons }` on the delegation, as a **plain ledger command** from this node's member credentials. No proposal, no confirmation, no execute round. On success the coupons leave the unassigned scan and the tick moves to the next chunk. A chunk that fails is handled by **cause, not uniformly** — the two causes want opposite responses:

- **A stale view** — another assigner committed first, or the coupon expired (§10). Only a fresh read fixes it, which the next tick performs, so the tick **ends**. Retrying smaller cannot help: a smaller chunk is not a newer view. Chunks already committed stand and the coupons left behind keep their full TTL, so the cost is at most one interval of latency. This is the common case: on a 3-assigner deployment two nodes lose every round.
- **A rejected command** — the ledger refused this batch's contents. A fresh read changes nothing, so the drain re-submits that chunk's coupons **one at a time**, logs each one the ledger still refuses at ERROR, and carries on with the next chunk. splice fetches and validates every `additionalCoupon`, so the culprit can sit anywhere in the chunk. Submitting each coupon alone pays every healthy one without having to locate the culprit first. Dropping only the primary would instead advance one coupon per tick.

One rejected chunk of `n` coupons costs `1 + n` submissions. That suits the causes a rejection actually has, because every one of them is correlated across coupons rather than isolated to a single coupon: a package skew on `RewardCouponV2`, a split splice refuses, a choice context a later coupon version requires. Most or all of a chunk then fails together, so the ERROR lines name every coupon at fault instead of one arbitrary member of the batch.

The two are told apart by the **Canton error id**, never the gRPC status code: benign contention arrives as `ABORTED`, `NOT_FOUND` *and* `FAILED_PRECONDITION` depending on which error fires, so a code-based rule would fan out a chunk another node had just assigned correctly. Canton's own retryability flag answers a different question — it marks those categories non-retryable because *resubmitting* cannot work, while this automation re-reads, which does fix them.

The skip lasts **one tick**. There is no cross-tick quarantine, so the drain stays stateless, a misclassified failure costs one tick instead of stranding a healthy coupon until restart, and a genuinely un-exerciseable coupon keeps producing an ERROR every tick for alerting to catch.
...
- **Coupon expires before assignment commits:** it leaves the ACS; the `Delegation_Assign` exercise for a chunk containing it fails cleanly (all-or-nothing); the tick ends and the next one re-reads, picking up the survivors. Assigning on first sight leaves the whole 36h TTL as margin, and the expiry margin (§9, step 3) keeps a coupon that is about to vanish out of a chunk in the first place.
- **A coupon that can never be exercised** heads the most-urgent-first order on every tick. It is contained twice over. The reader admits only contracts created by `Splice.Amulet:RewardCouponV2`, which is exactly what `Delegation_Assign` fetches, so another implementation of the `RewardCoupon` interface cannot enter a batch at all (§9 step 2). Should one still be rejected — a package skew on `RewardCouponV2` itself being the residual case — the drain submits each coupon in that chunk on its own and pays every healthy one (§9 step 5). The coupon itself is still lost, and its id is logged at ERROR every tick for alerting and human diagnosis (issue #278).
- **Coupon assigned by another node mid-tick:** same all-or-nothing failure; harmless. The tick ends and the next one re-reads a fresh view, rather than retrying a set another node has already taken.
- **No active delegation:** no-op with a clear log (automation is off for that decparty until governance creates one).
- **More than one active delegation:** take the newest by created-event offset and warn. That is the one the most recent vote produced, so it is the split the decparty last agreed. Refusing instead would stop assignment entirely and lose every coupon to expiry, which is the worse outcome for a state that arises transiently during a create-then-revoke replacement (§8). The offset comes from the ledger, so every node picks the same contract. The stale delegation stays exercisable until it is revoked — see §12.
- **This node not in `assigners`:** cannot assign; log and skip. Liveness still holds as long as one listed assigner's node is up.
- **Transient delegation-view skew during replacement:** while the split is being changed (§8), the automation sees zero delegations (no-op) or two (newest wins). It never assigns to a stale split: with an atomic replacement the old one is already archived, and in the two-delegation window the newest is the one the latest vote produced.
- **Coupon-ACS size:** the read is a fold, not a list — each coupon is decoded and filtered as it streams, and the unassigned-only result is a cid plus an expiry. `read_oldest_expiry` keeps no list at all, only a running `min`. So node memory tracks the *unassigned* subset rather than the whole coupon ACS, which is what the earlier collect-then-filter read made it (#413).
- **Chunk size:** bounded by output creates, not a fixed coupon count (§9, step 4), and operator-tunable (`reward_max_creates`) because the ledger's transaction ceiling is unmeasured. A tick drains its whole assignable set, so a long interval cannot by itself build a backlog.
...
- **No agent is trusted for the split *a given delegation applies*.** The split is a field on the delegation and `Delegation_Assign` reads it directly (`newBeneficiaries = split`); the caller supplies only coupon ids, and the coupon it names is pinned to splice's `RewardCouponV2` with a matching `dso` (§7) so a look-alike implementation cannot borrow the decparty's authority. For one delegation, fairness is an L3 invariant enforced **by construction**.
- **But *which* delegation is authoritative is NOT an L3 guarantee.** Canton cannot enforce a per-decparty singleton: contract keys do not provide cross-participant uniqueness, and `executeImpl` cannot query the ACS to see whether one already exists. Replacement is atomic when `priorDelegation` is supplied (§8), so the failure needs an omission rather than an attack — a `Setup` vote that leaves `priorDelegation` empty while one is live. The consequence is real: an assigner can still exercise the superseded delegation, assigning coupons to the **old** split. The automation itself takes the newest (§11), so it does not compound the mistake, but nothing on the ledger stops a direct submit against the stale one. What guards it is a propose-time rejection in DecMan (`propose_action` refuses a null `priorDelegation` when one is active) — **L2**, and a direct ledger submit bypasses it. This is the one place the design's L3-only claim does not hold, and it is a governance convention, not a construction.
...
- **Detection is required, not optional.** Several loss modes are silent by construction: a coupon the automation cannot see (`providerIsObserver = false`, set by the DSO when the provider's vetting state disallows it) and a coupon that can never be exercised both look identical to "nothing to assign" in the logs. Monitoring is what separates them (issue #278); until it exists, a stalled engine is indistinguishable from an idle one.
...
- **Rust unit:** `ProposalType::SetupCouponReassignmentDelegation` serialization round-trip (copy the `build_proposal_*_shape` test shape); `validate()` rejects empty set / bad percentages / >cap (exact `DamlDecimal`, via `validate_reward_beneficiaries`); selection (`select_assignable` — expiry margin, most-urgent-first, no truncation, and a coupon kept even when the beneficiary may lack time to mint) and chunk sizing (`chunk_size` — creates-based, never 0); delegation selection (`newest_delegation_for` — newest by offset regardless of read order, another decparty's delegation ignored); the per-tick gating (zero delegations → no-op; node-not-assigner → skip).
