...
1. **Read the active delegation.** Load `CouponReassignmentDelegation` for the decparty via `active_created_records`. Zero ⇒ no-op; more than one ⇒ take the newest by created-event offset and warn. This node's member party must appear in `assigners` (else it cannot assign — log and skip).
