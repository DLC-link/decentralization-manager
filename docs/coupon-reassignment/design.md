...
- **A rejected command** — the ledger refused this batch's contents. A fresh read changes nothing, so the drain **bisects** the chunk to find the one coupon at fault, logs it at ERROR, and carries on with the rest. Halving is what finds it: splice fetches and validates every `additionalCoupon`, so the culprit can sit anywhere in the chunk, and dropping only the primary would advance one coupon per tick. Each half that commits is real progress.

The two are told apart by the **Canton error id**, never the gRPC status code: benign contention arrives as `ABORTED`, `NOT_FOUND` *and* `FAILED_PRECONDITION` depending on which error fires, so a code-based rule would bisect a chunk another node had just assigned correctly. Canton's own retryability flag answers a different question — it marks those categories non-retryable because *resubmitting* cannot work, while this automation re-reads, which does fix them.
...
- **A coupon that can never be exercised** heads the most-urgent-first order on every tick. It is contained twice over. The reader admits only contracts created by `Splice.Amulet:RewardCouponV2`, which is exactly what `Delegation_Assign` fetches, so another implementation of the `RewardCoupon` interface cannot enter a batch at all (§9 step 2). Should one still be rejected — a package skew on `RewardCouponV2` itself being the residual case — the drain bisects it out and pays every other coupon (§9 step 5). The coupon itself is still lost, and its id is logged at ERROR every tick for alerting and human diagnosis (issue #278).
