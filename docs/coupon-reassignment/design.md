...
**In scope**
- A new DAML template `CouponReassignmentDelegation` (in the `governance-rewards-automation-v1-rc1` package): signed by the decparty, holding the fixed `[RewardBeneficiary]` split, the DSO party whose coupons it may assign, and the assigner list, with a `nonconsuming` `Delegation_Assign` choice that assigns caller-supplied coupons to the baked-in split (§7).
...
- **But *which* delegation is authoritative is NOT an L3 guarantee.** Canton cannot enforce a per-decparty singleton: contract keys do not provide cross-participant uniqueness, and `executeImpl` cannot query the ACS to see whether one already exists. Replacement is atomic when `priorDelegation` is supplied (§8), so the failure needs an omission rather than an attack — a `Setup` vote that leaves `priorDelegation` empty while one is live. The consequences are real: the automation then refuses to act on an ambiguous split and **every coupon expires**, and an assigner can still exercise the superseded delegation, assigning coupons to the **old** split. What guards it is a propose-time rejection in DecMan (`propose_action` refuses a null `priorDelegation` when one is active) plus the automation's refusal — both **L2**. A direct ledger submit bypasses the first. This is the one place the design's L3-only claim does not hold, and it is a governance convention, not a construction.
...
- **Detection is required, not optional.** Several loss modes are silent by construction: an ambiguous delegation, a coupon the automation cannot see (`providerIsObserver = false`, set by the DSO when the provider's vetting state disallows it), and a coupon that can never be exercised all look identical to "nothing to assign" in the logs. Monitoring is what separates them (issue #278); until it exists, a stalled engine is indistinguishable from an idle one.
...
- **Rust unit:** `ProposalType::SetupCouponReassignmentDelegation` serialization round-trip (copy the `build_proposal_*_shape` test shape); `validate()` rejects empty set / bad percentages / >cap (exact `DamlDecimal`, via `validate_reward_beneficiaries`); selection (`select_assignable` — expiry margin, most-urgent-first, no truncation, and a coupon kept even when the beneficiary may lack time to mint) and chunk sizing (`chunk_size` — creates-based, never 0); the per-tick gating (zero delegations → no-op; two → refuse; node-not-assigner → skip).
...
3. **Package placement** of `CouponReassignmentDelegation` (and the `SetupCouponReassignmentDelegation` / revoke actions): the `governance-rewards-automation-v1-rc1` package — renamed from `governance-rewards-v1` after team alignment, since the required `dso` field is not an SCU-compatible upgrade of it.
...
- **M1 — DAML delegation + tests:** `CouponReassignmentDelegation` (template + `Delegation_Assign` + `Delegation_Revoke`) in the `governance-rewards-automation-v1-rc1` package + `daml.yaml` dep on `splice-api-reward-assignment-v1` + the DAML tests in §13. Reviewable in isolation; the `Delegation_Assign` security review lands here.
