...
| `DECPM_TENANT_ACS_MAX_BYTES` | Ceiling on an ACS snapshot the wallet relays over the tenant API. The Noise chunked-transfer limit does not bound that path; it goes over HTTP in ranges. **Export-side only.** The export streams to disk, so this bounds a file on the source — but the joiner reads the staged snapshot whole into memory, so it is still a memory commitment there | `536870912` (512 MiB) |
