...
| `DECPM_TENANT_ACS_MAX_BYTES` | Ceiling on an ACS snapshot the wallet relays over the tenant API. The Noise chunked-transfer limit does not bound that path; it goes over HTTP. **Export-side only**, and while the snapshot travels in one JSON body the effective ceiling is lower than this: base64 inflates by 4/3 against a fixed 100 MiB request limit, so roughly 75 MiB of ACS. Raising it past that buys nothing until the transfer moves in ranges. Assembled in memory on both ends, so raise deliberately | `536870912` (512 MiB) |
