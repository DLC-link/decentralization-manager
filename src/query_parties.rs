...
use canton_proto_rs::com::digitalasset::canton::{
    crypto::admin::v30::{ListMyKeysRequest, vault_service_client::VaultServiceClient},
    topology::admin::v30::{
...
pub async fn query_parties(config: &NodeConfig, party_id_prefix: &str) -> Result {
    ...
    let mut namespace_key_fingerprints = std::collections::HashMap::new();
    for key_meta in keys_response.private_keys_metadata {
        if let Some(pub_key_with_name) = &key_meta.public_key_with_name {
            if let Some(pub_key) = &pub_key_with_name.public_key {
                if let Some(canton_proto_rs::com::digitalasset::canton::crypto::v30::public_key::Key::SigningPublicKey(signing_key)) = &pub_key.key {
    ...
    for (idx, result) in response.results.iter().enumerate() {
        if let Some(item) = &result.item {
            ...
            let party_id = format!("{party_id_prefix}::{}", item.decentralized_namespace);
            println!(
                "\n    {} {}",
            ...
            let p2p_response = client.list_party_to_participant(p2p_request).await?.into_inner();
            ...
            if let Some(p2p_result) = p2p_response.results.first() {
                if let Some(p2p_item) = &p2p_result.item {
                    ...
                    for (i, p) in p2p_item.participants.iter().enumerate() {
                        println!("      {} {}", format!("[{i}]").dimmed(), p.participant_uid.blue());
                    }
                }
            } else {
