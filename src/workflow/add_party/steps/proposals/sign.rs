...
use crate::{
    config::NodeConfig, consts::SIGNED_ADD_PARTY_PROPOSALS_PREFIX, error::Result, utils,
    workflow::add_party::AddPartyDirs,
};

/// Sign add party proposals
///
/// Both existing members AND the new member sign both proposals.
/// The new member must sign to prove consent to being added to the party.
...
pub async fn sign_proposals(
    ...
    tracing::debug!("Calling SignTransactions RPC for add party proposals...");
    ...
    fs::create_dir_all(&dirs.add_party_signed_dir).await?;
