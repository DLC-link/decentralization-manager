...
pub struct AppState {
    ...
    pub add_party_trigger: Arc<Notify>,
...
pub async fn start_server(host: &str, port: u16, config: NodeConfig, test_mode: bool) -> Result {
    ...
    let add_party_trigger = Arc::new(Notify::new());
    ...
    let app_state = web::Data::new(AppState {
        ...
        add_party_trigger: add_party_trigger.clone(),
    ...
    let add_party_state = web::Data::new(Arc::new(handlers::AddPartyWorkflowState::new()));
    ...
    // Start attestor trigger listener for add party (starts attestor workflow when add party invite received)
    let add_party_attestor_config = config.clone();
    let add_party_attestor_control = listener_control.clone();
    let add_party_attestor_notify = listener_notify.clone();
    let add_party_attestor_state = add_party_state.clone();
    let add_party_coordinator_pubkey = coordinator_pubkey.clone();
    tokio::spawn(async move {
        run_add_party_attestor_listener(
            add_party_attestor_config,
            add_party_attestor_control,
            add_party_attestor_notify,
            add_party_attestor_state,
            add_party_trigger,
            add_party_coordinator_pubkey,
        )
        .await;
    });
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .app_data(add_party_state.clone())
                ...
                if let Ok(msg) = Message::from_bytes(&body_bytes) {
                    ...
                    match msg.msg_type {
                        ...
                        MessageType::InviteAddParty => {
                            tracing::info!(
                                "Received add party invite, storing as pending invitation"
                            );
                            if let Some(ref pubkey) = peer_pubkey_hex {
                                let invitation = PendingInvitation {
                                    id: format!("add-party-{}", &pubkey[..16]),
                                    invitation_type: InvitationType::AddParty,
                                    coordinator_pubkey: pubkey.clone(),
                                    coordinator_name: None,
                                    received_at: std::time::SystemTime::now()
                                        .duration_since(std::time::UNIX_EPOCH)
...
/// Background task that starts add party attestor workflow when triggered by an invite
async fn run_add_party_attestor_listener(
    config: NodeConfig,
    listener_control: Arc<RwLock<ListenerControl>>,
    listener_notify: Arc<Notify>,
    add_party_state: web::Data<Arc<handlers::AddPartyWorkflowState>>,
    add_party_trigger: Arc<Notify>,
    coordinator_pubkey: Arc<RwLock<Option<String>>>,
) {
    loop {
        // Wait for trigger
        add_party_trigger.notified().await;

        tracing::info!("Received add party invite, starting add party attestor workflow...");

        // Check if already in progress
        {
            let status = add_party_state.status.read().await;
            if *status == types::WorkflowProgress::InProgress {
                tracing::warn!("Already in add party workflow, ignoring invite");
                continue;
            }
        }
        ...
        // Start add party attestor workflow
        let workflow_config = config.clone();
        let result = workflow::start_attestor(workflow_config, coordinator).await;
        ...
        match result {
            Ok(()) => {
                *status = types::WorkflowProgress::Completed;
                tracing::info!("Add party attestor workflow completed successfully");
            }
            Err(e) => {
                *status = types::WorkflowProgress::Failed;
                *error = Some(format!("{e}"));
                tracing::error!("Add party attestor workflow failed: {e}");
            }
        }
    }
}
