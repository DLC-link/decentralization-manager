...
# - Canton ledger:  5001/5011/5021 (tunneled to participant-devnet-{1,2,3}
...
_canton_forward_loop() {
    ...
    while true; do
        kubectl --context="$KUBE_CONTEXT_DEVNET" port-forward \
            "svc/participant-devnet-$idx" -n "$KUBE_NS_CANTON" \
...
start_canton_tunnels() {
    ...
    echo "All 6 Canton ports forwarded (5001/5002, 5011/5012, 5021/5022 → svc/participant-devnet-{1,2,3})."
}
...
stop_canton_tunnels() {
    if [ "${#CANTON_TUNNEL_PIDS[@]}" -gt 0 ]; then
        ...
        # an EXIT_RUNSH=0 run #9 left three svc/participant-devnet-*
        ...
        # The -f pattern is scoped to our devnet context + devnet
        # service so it won't affect unrelated kubectl invocations.
        pkill -f "kubectl --context=$KUBE_CONTEXT_DEVNET port-forward svc/participant-devnet" 2>/dev/null || true
        CANTON_TUNNEL_PIDS=()
    fi
}
