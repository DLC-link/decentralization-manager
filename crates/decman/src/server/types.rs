    ...
    pub fn validate(&self) -> Result<(), String> {
        match self {
            ...
            | ActionType::GovernanceSetThreshold { new_threshold } => {
                validate_threshold(*new_threshold).map_err(|e| e.to_string())
            }
            ActionType::GovernanceSetTimeout {
                new_timeout_microseconds,
            } => validate_timeout(*new_timeout_microseconds).map_err(|e| e.to_string()),
            ...
            | ActionType::ProcessorDeploymentRequest {
                ...
                ..
            } => validate_beneficiary_weights(&far.beneficiaries).map_err(|e| e.to_string()),
            ActionType::VaultUpdateFarBeneficiaries {
                new_beneficiaries, ..
            } => validate_beneficiary_weights(new_beneficiaries).map_err(|e| e.to_string()),
            _ => Ok(()),
        }
    }
}
